...
from helper import train_logistic_regression, evaluate_model,balance_dataset,preprocess_data,train_rf,train_sgd


def run_model(model,down_samp,results_file,adata,y,le):
    """
    ...
    """
    X = adata.X
    #print(model,down_samp,results_file,adata,y,le)
    if down_samp == False :
    ...
    if model == "lr":
        ...
        evaluate_model(lr, X_train, y_train, le,"train",model,down_samp,results_file)
        evaluate_model(lr, X_test, y_test, le,"test",model,down_samp,results_file)
    elif model == "rf":
        rf = train_rf(X_train, y_train)
        evaluate_model(rf, X_train, y_train, le,"train",model,down_samp,results_file)
        evaluate_model(rf, X_test, y_test, le,"test",model,down_samp,results_file)
    elif model == "sgd":
...
if __name__ == "__main__":
    ...
    run_model(args.model, args.down_samp,results_file, adata, y,le)
